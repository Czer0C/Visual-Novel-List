...
import { getUserList, getVisualNovel } from "../../lib/vndb/index";
import { VisualNovel, VNInUserList } from "../../lib/vndb/types";
...
export default async function getFullList(
  _: NextApiRequest,
  res: NextApiResponse
) {
  try {
    ...
    try {
      for (let i = 0; i < pages; i++) {
        ...
        const getCurrentBatch: VNDBResponse = await getVisualNovel(
          currentBatchIDs
        );
...
const joinAndClean = (uList: VNInUserList[], vns: VisualNovel[]) => {
  const fullList = uList.map((base, index) => {
    ...
    return {
      ...
      vote: base.vote / 10 || -1,
      voted: base.vote
        ? moment(base.voted * 1000).format("MM/DD/YYYY")
        : "Unrated",
      vn: formatedVN,
    };
  });
